...
src/App.jsx                Real routing via react-router-dom (BrowserRouter) + AuthProvider +
                           I18nProvider, defers every data concern to src/containers/. Every
                           page is a real URL (/live, /leagues, /match/:id, etc.) — no more
                           in-memory view-switching, browser Back/Forward and direct links work.
...
4. Routing is already real (`react-router-dom`'s `BrowserRouter`, one route
   per page) — if you're integrating this into a larger app with its own
   router, replace `App.jsx`'s `<BrowserRouter>` with your own and keep the
   `<Routes>` tree; every container still takes the same props regardless.
5. Persian text renders via `useT()` (see "Translation / i18n" below) — the
   `font-display`/`font-body` Tailwind classes fall back to Vazirmatn
   per-glyph, so mixed EN/FA content (e.g. an English team name inside a
   Persian sentence) renders correctly automatically. **Layout does not
   mirror for Persian** — `dir` is hardcoded to `"ltr"` always, by explicit
   product decision; only the text itself changes with the language toggle.
   (An earlier pass built full RTL layout mirroring, which worked correctly
   — see "RTL — how it's implemented" below — before this decision reversed
   it. That section is kept for reference, but `dir="rtl"` is no longer
